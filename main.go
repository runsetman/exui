...
func updateSetting(port int, username string, password string, hostIp string, publicKey string, privateKey string) {
	err := database.InitDB(config.GetDBPath())
	if err != nil {
		fmt.Println(err)
		return
	...
	if hostIp != "" {
		err := settingService.SetHostIp(hostIp)
		if err != nil {
			fmt.Println("set host ip failed:", err)
		} else {
			fmt.Printf("set host ip %v success", hostIp)
		}
	}
	...
	if publicKey != "EMPTY" || privateKey != "EMPTY" {
		...
		if err != nil {
			...
			fmt.Printf("set public key %v success\n", publicKey)
		}

		err = settingService.SetKeyFile(privateKey)
		if err != nil {
			fmt.Println("set private key failed:", err)
		} else {
			fmt.Printf("set private key %v success\n", privateKey)
		}
	}
}
...
func main() {
	...
	settingCmd.StringVar(&hostIp, "hostIp", "", "set host ip")
	settingCmd.StringVar(&publicKey, "publickey", "", "set public key")
	settingCmd.StringVar(&privateKey, "privatekey", "", "set private key")
	...
	case "setting":
		...
		if reset {
			...
			updateSetting(port, username, password, hostIp, publicKey, privateKey)
		}
		if show {
			showSetting(show)
		}
